...
def asr_mp3_to_srt(state: WorkflowState) -> WorkflowState:
    ...
    output_srt = config_path(config, "paths.asr_srt")
    asr_result = transcribe_mp3_to_srt(input_mp3, output_srt, config)
    report_path = config_path(config, "paths.reports_dir") / "asr_report.json"
    write_asr_report(report_path, asr_result)
    state["asr_result"] = {
        key: value for key, value in asr_result.items() if key not in {"cues", "srt"}
    }
