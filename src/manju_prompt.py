...
3. 核心设定：重生、复仇、契约婚姻、隐藏身份、失忆、替身、联姻、豪门争斗、修仙境界、宗门矛盾等。
...
5. 每条问题的检测信息，例如：
   ...
   * zh_text：对应中文
   * en_text：当前英文
   * start_time / end_time：时间轴
   * duration：字幕时长
   * tts_duration：真实 TTS 音频时长
   * word_count：英文词数
   * wpm：估算语速
   * issue_types：问题类型列表
...
* too_long：TTS 音频明显超出字幕时长
* high_wpm：词数过多，语速过快
* fragment：英文残句，不适合单独朗读
* unnatural：英文不自然或有中式英语
* mistranslation：明显误译
* missing_key_info：删掉了关键剧情信息
* missing_identity：删掉了关键身份或人物关系
* missing_hook：削弱了钩子、爽点或反转
* name_inconsistent：人名、称呼或身份翻译前后不一致
* repeated_phrase：相邻字幕重复同一短语，听感机械
* bad_tts_style：不适合 TTS 朗读
* punctuation_issue：标点或编码问题
...
2. 人物关系：夫妻、前任、师徒、亲生/收养、真假千金、主仆、上下级、仇人、暗恋等。
3. 核心设定：重生、复仇、契约婚姻、隐藏身份、替身、联姻、豪门争斗、修仙境界、宗门矛盾、失忆等。
